{{-- resources/views/return-requests/create.blade.php --}}
{{-- Form buat return request — Tailwind + Alpine.js --}}
...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-bold text-gray-800">Buat Return Request</h2>
                <p class="text-sm text-gray-500">Ajukan pengembalian barang ke gudang utama</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        ...
        {{-- ═══════════════════════════════════════════════════════════════════
             Alpine.js Component: returnForm
             Handles: pilih lab → fetch aset via AJAX → dynamic item rows
             ═══════════════════════════════════════════════════════════════════ --}}
        <div x-data="{
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <select name="lab_id"
                                    class="w-full text-sm border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('lab_id') border-red-400 @enderror"
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        {{-- Placeholder: belum pilih lab --}}
                        <div x-show="!labId && !loading" class="py-10 text-center text-gray-400 text-sm">
                            Pilih laboratorium terlebih dahulu
                        </div>

                        {{-- Placeholder: tidak ada stok --}}
                        <div x-show="labId && !loading && availableAssets.length === 0"
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500">Stok Lab</th>
                                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 w-24">Qty</th>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Kondisi</th>
                                    ...
                                    <template x-for="(item, index) in items" :key="index">
                                        <tr :class="overStock(item) ? 'bg-red-50' : ''">
                                            ...
                                            <td class="px-4 py-2">
                                                ...
                                                <p x-show="item.condition === 'damaged'"
                                                   class="text-yellow-600 text-xs mt-0.5">
                                                    Tidak kembali ke stok baik
                                                </p>
                                                <p x-show="item.condition === 'lost'"
