...
<div class="bg-white rounded-2xl p-6 shadow-sm">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Filter --}}
            <button class="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <tr class="border-b border-gray-100 hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($request->status === 'Pending')
                                <span class="inline-block px-4 py-1 bg-yellow-400 text-yellow-900 text-xs font-semibold rounded-md">
                                    Pending
                                </span>
                            @elseif ($request->status === 'Approved')
                                <span class="inline-block px-4 py-1 bg-green-600 text-white text-xs font-semibold rounded-md">
                                    Approved
                                </span>
                            @elseif ($request->status === 'Rejected')
                                <span class="inline-block px-4 py-1 bg-red-600 text-white text-xs font-semibold rounded-md">
                                    Rejected
                                </span>
                            @else
                                <span class="inline-block px-4 py-1 bg-gray-200 text-gray-600 text-xs font-semibold rounded-md">
                                    {{ $request->status }}
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div style="display:flex; align-items:center; justify-content:center; gap:12px;">
                                ...
                                {{-- Tombol Delete --}}
                                <form
                                    action="{{ route('requestlab.destroy', $request->id) }}"
                                    method="POST"
...
<div
    ...
    <div style="background:#fff; border-radius:16px; width:100%; max-width:760px; margin:0 16px; overflow:hidden; box-shadow:0 20px 60px rgba(0,0,0,0.15);">
        ...
        <div style="padding:16px 32px 24px 32px;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:20px;">
                ...
                <div>
                    ...
                    <table style="width:100%; font-size:13px; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden; border-collapse:separate; border-spacing:0;">
                        <thead>
                            <tr style="background:#f3f4f6;">
                                <th style="padding:8px 14px; text-align:left; font-weight:500; color:#374151; border-bottom:1px solid #e5e7eb;">Asset Name</th>
                                <th style="padding:8px 14px; text-align:left; font-weight:500; color:#374151; border-bottom:1px solid #e5e7eb;">Qty</th>
                            </tr>
                        </thead>
...
<script>
    function openModal(requestId) {
        ...
        document.getElementById('modal_electronic').innerHTML    = '<tr><td colspan="2" style="padding:12px;text-align:center;color:#9ca3af;font-size:12px;">Memuat...</td></tr>';
        document.getElementById('modal_nonelectronic').innerHTML = '<tr><td colspan="2" style="padding:12px;text-align:center;color:#9ca3af;font-size:12px;">Memuat...</td></tr>';
        ...
        .catch(() => {
            document.getElementById('modalProgress').style.width = '100%';
            const errHtml = '<tr><td colspan="2" style="padding:12px;text-align:center;color:#f87171;font-size:12px;">Gagal memuat data</td></tr>';
