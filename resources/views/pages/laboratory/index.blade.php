...
// encode labs data for view modal JS
$labsJson = $laboratories->map(fn($lab) => [
    'id'       => $lab->id,
    'lab_name' => $lab->lab_name,
    'capacity' => $lab->capacity,
    'staff'    => $lab->users->map(fn($u) => [
        'name' => $u->name,
        'role' => $u->role,
    ])->values(),
    'assets'   => $lab->assets->map(fn($a) => [
        'name'     => $a->asset_name,
        'category' => $a->asset_category,
        'stock'    => $a->pivot->total_asset_lab,
    ])->values(),
])->keyBy('id');
@endphp
...
{{-- ══════════════════════════════════════════
     MODAL VIEW LABORATORY (read-only, 2 steps)
══════════════════════════════════════════ --}}
<div id="modal-view-lab" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.45); z-index:50; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:16px; width:100%; max-width:560px; margin:0 16px; box-shadow:0 20px 60px rgba(0,0,0,0.15);">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; padding:20px 24px; border-bottom:1px solid #e5e7eb;">
            <h3 id="view-lab-title" style="font-size:16px; font-weight:700; color:#111827; margin:0;">View Laboratory</h3>
            <button onclick="closeViewModal()" style="background:none; border:none; cursor:pointer; color:#9ca3af; font-size:22px;">&times;</button>
        </div>

        <div style="display:flex; padding:16px 24px 0; gap:6px;">
            <div id="vlab-bar-1" style="flex:1; height:4px; border-radius:2px; background:#111B4C;"></div>
            <div id="vlab-bar-2" style="flex:1; height:4px; border-radius:2px; background:#e5e7eb;"></div>
        </div>

        {{-- View Step 1: General Info --}}
        <div id="vlab-step-1" style="padding:24px;">
            <h4 style="font-size:15px; font-weight:600; color:#374151; margin:0 0 20px; text-align:center;">General Information</h4>
            <div style="margin-bottom:16px;">
                <label style="font-size:13px; font-weight:500; color:#6b7280; display:block; margin-bottom:6px;">Laboratory Name:</label>
                <div id="vlab-name" style="border:1px solid #e5e7eb; border-radius:8px; padding:10px 14px; font-size:14px; background:#f9fafb; color:#374151;"></div>
            </div>
            <div style="margin-bottom:16px;">
                <label style="font-size:13px; font-weight:500; color:#6b7280; display:block; margin-bottom:6px;">PC Capacity:</label>
                <div id="vlab-capacity" style="border:1px solid #e5e7eb; border-radius:8px; padding:10px 14px; font-size:14px; background:#f9fafb; color:#374151;"></div>
            </div>
            <div style="display:flex; justify-content:space-between; gap:8px; margin-top:24px;">
                ...
                <button type="button" onclick="vlabGoStep(2)"
        ...
        {{-- View Step 2: Asset Info --}}
        <div id="vlab-step-2" style="display:none; padding:24px; max-height:55vh; overflow-y:auto;">
                ...
                <table style="width:100%; border-collapse:collapse; font-size:13px;">
                    <thead>
                        <tr style="background:#f9fafb;">
                            <th style="padding:10px 14px; text-align:left; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Nama Barang</th>
                            <th style="padding:10px 14px; text-align:left; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Kategori</th>
                            <th style="padding:10px 14px; text-align:left; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Jumlah Stok</th>
                            <th style="padding:10px 14px; text-align:left; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Jumlah Rusak</th>
                            <th style="padding:10px 14px; text-align:left; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Jumlah Hilang</th>
                        </tr>
                    </thead>
                    <tbody id="vlab-asset-tbody">
                        <tr><td colspan="5" style="text-align:center; padding:24px; color:#9ca3af;">Belum ada aset</td></tr>
                    </tbody>
                </table>
            </div>
            <div style="display:flex; justify-content:space-between; gap:8px; margin-top:24px;">
                <button type="button" onclick="vlabGoStep(1)"
                ...
                <button type="button" onclick="closeViewModal()"
                        style="background:#111B4C; color:#fff; border:none; border-radius:8px; padding:9px 20px; font-size:13px; cursor:pointer; font-weight:600;">Close</button>
            </div>
        </div>
