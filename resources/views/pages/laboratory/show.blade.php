...
<div class="db-wrap">
    ...
    <div id="section-pc" class="db-card" style="padding:0; overflow:hidden;">
        ...
        <div style="overflow-x:auto;">
            <table class="db-table" style="min-width:1100px;">
                <thead>
                    <tr>
                        <th>No PC</th>
                        <th>Type</th>
                ...
                <tbody>
                        ...
                        <td>
                            <div class="action-btns">
                                ...
                                <form method="POST" action="{{ route('pc.destroy', [$laboratory->id, $pc->id]) }}"
                                      onsubmit="return confirm('Hapus PC ini? Capacity lab akan berkurang.')" style="display:inline">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="11" style="text-align:center; padding:32px; color:#9ca3af; font-size:13px;">Belum ada PC</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{-- PC section bottom: Back (left) + Next (right) --}}
        <div style="padding:16px 24px; display:flex; justify-content:space-between; align-items:center; border-top:1px solid #f3f4f6;">
            ...
            <button onclick="showSection('asset')"
    ...
    <div id="section-asset" style="display:none;" class="db-card" style="padding:0; overflow:hidden;">
        <div style="display:flex; align-items:center; justify-content:space-between; padding:18px 24px 14px; border-bottom:1px solid #f3f4f6;">
        ...
        <div style="overflow-x:auto;">
            <table class="db-table" style="min-width:900px;">
                <thead>
                    <tr>
                        <th>Nama Aset</th>
                        <th>Kategori</th>
                ...
                <tbody>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" style="text-align:center; padding:32px; color:#9ca3af; font-size:13px;">Belum ada aset di lab ini</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{-- Asset section bottom: Back only --}}
        <div style="padding:16px 24px; border-top:1px solid #f3f4f6;">
            <button onclick="showSection('pc')"
...
<div id="modal-edit-pc" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.45); z-index:50; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:16px; width:100%; max-width:500px; margin:0 16px; box-shadow:0 20px 60px rgba(0,0,0,0.15); max-height:90vh; display:flex; flex-direction:column;">
        ...
        <form method="POST" id="editPcForm" style="overflow-y:auto; flex:1; padding:24px; display:flex; flex-direction:column; gap:14px;">
            ...
            @foreach(['processor'=>'Processor','ram'=>'RAM','ssd'=>'SSD','motherboard'=>'Motherboard','vga'=>'VGA','cpu_fan'=>'CPU Fan','powersupply'=>'Power Supply'] as $f => $l)
            <div>
                <label style="font-size:13px; font-weight:500; color:#374151; display:block; margin-bottom:6px;">{{ $l }}</label>
                <input type="text" name="{{ $f }}" id="epc_{{ $f }}" placeholder="Enter here..."
                       style="width:100%; border:1px solid #d1d5db; border-radius:8px; padding:10px 14px; font-size:13px; outline:none; box-sizing:border-box;">
            </div>
            @endforeach
            <div style="display:flex; justify-content:flex-end; gap:8px; padding-top:4px;">
...
<div id="modal-add-asset" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.45); z-index:50; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:16px; width:100%; max-width:500px; margin:0 16px; box-shadow:0 20px 60px rgba(0,0,0,0.15); max-height:90vh; display:flex; flex-direction:column;">
        ...
        <form method="POST" action="{{ route('laboratory.update', $laboratory->id) }}" style="overflow-y:auto; flex:1; padding:24px; display:flex; flex-direction:column; gap:14px;">
            @csrf @method('PUT')
            {{-- pass existing pc data agar tidak terhapus --}}
...
document.getElementById('modal-add-pc').addEventListener('click', e => { if(e.target===e.currentTarget) closeAddPcModal(); });
...
function openEditPcModal(pcId,index,type,status,processor,ram,ssd,motherboard,vga,cpuFan,ps) {
    ...
    document.getElementById('epc_processor').value   = processor;
    document.getElementById('epc_ram').value         = ram;
    document.getElementById('epc_ssd').value         = ssd;
    document.getElementById('epc_motherboard').value = motherboard;
    document.getElementById('epc_vga').value         = vga;
    document.getElementById('epc_cpu_fan').value     = cpuFan;
    document.getElementById('epc_powersupply').value = ps;
...
document.getElementById('modal-edit-pc').addEventListener('click', e => { if(e.target===e.currentTarget) closeEditPcModal(); });
...
document.getElementById('modal-add-asset').addEventListener('click', e => { if(e.target===e.currentTarget) closeAddAssetModal(); });
...
document.getElementById('modal-edit-lab').addEventListener('click', e => { if(e.target===e.currentTarget) closeEditLabModal(); });
</script>
@endpush
