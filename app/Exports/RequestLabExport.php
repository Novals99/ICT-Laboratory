    ...
    public function collection(): Collection
    {
        return RequestLab::with(['user', 'lab', 'request_items'])
            ...
            ->map(fn ($request) => [
                ...
                'request_date'  => $request->request_date ? \Carbon\Carbon::parse($request->request_date)->format('d-m-Y') : '-',
                'status'        => $request->request_status === 'partial' ? 'Partially Approved' : ucwords($request->request_status),
            ]);
    }
